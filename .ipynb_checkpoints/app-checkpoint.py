...
def process_single_image(image_stream, date_str, start_time_str, config):
    ...
    ocr_data, cropped_chart_paths = {}, {}; img = Image.open(image_stream)
    if not DEBUG_MODE_SKIP_OCR:
        llm_result_dict = ocr_with_llm(img);
        if llm_result_dict: ocr_data.update({k: v for k, v in llm_result_dict.items() if v is not None})
    if 'chart_crops' in config:
        ...
        if IS_PRODUCTION:
            ...
            except Exception as e: print(f"!!! S3 客户端初始化失败: {e}")
        for key, crop_area in config['chart_crops'].items():
            if crop_area == (0, 0, 0, 0): continue;
            chart_img = img.crop(crop_area);
            safe_start_time = re.sub(r'[^a-zA-Z0-9_-]', '_', str(start_time_str)); unique_id = uuid.uuid4().hex[:8]; chart_filename = f"{date_str}_{safe_start_time}_{key}_{unique_id}.png"
            if IS_PRODUCTION and s3_client and S3_BUCKET:
                try:
                    in_mem_file = io.BytesIO(); chart_img.save(in_mem_file, format='PNG'); in_mem_file.seek(0);
                    s3_client.upload_fileobj(in_mem_file, S3_BUCKET, chart_filename, ExtraArgs={'ACL': 'public-read', 'ContentType': 'image/png'});
                    web_path = f"https://{S3_BUCKET}.s3.{os.environ.get('AWS_REGION')}.amazonaws.com/{chart_filename}"; cropped_chart_paths[key] = web_path
                except Exception as e: print(f"!!! 上传到 S3 失败: {e}")
            else:
                gen_folder = app.config.get('GENERATED_FOLDER', 'static/generated_charts/'); os.makedirs(gen_folder, exist_ok=True); save_path = os.path.join(gen_folder, chart_filename); chart_img.save(save_path);
                web_path = os.path.join('generated_charts', chart_filename).replace('\\', '/'); cropped_chart_paths[key] = web_path
...
def upload():
    if request.method == 'POST':
        ...
        temp_ocr_data = ocr_with_llm(Image.open(first_image_stream)) if not DEBUG_MODE_SKIP_OCR else {}; first_image_stream.seek(0)
        live_start_time = temp_ocr_data.get('live_start_time')
        if not live_start_time: flash('处理失败：未能从第一张图片中识别到“开播时间”，无法保存数据。', 'danger'); return redirect(url_for('daily_review'))
        existing_entry = LiveData.query.filter_by(live_date=live_date, live_start_time=live_start_time).first()
        if existing_entry: flash(f'日期 {live_date_str} (开播于 {live_start_time}) 的数据已存在。', 'warning'); return redirect(url_for('daily_review', date=live_date_str, start_time=live_start_time))
        configs_to_use = [IMAGE_1_CONFIG, IMAGE_2_CONFIG, IMAGE_3_CONFIG, IMAGE_4_CONFIG]; final_ocr_data, final_chart_paths = {}, {}
        final_ocr_data.update(temp_ocr_data or {});
        if len(files) < 4: flash(f'提醒：您上传了 {len(files)} 张图片，建议上传全部4张以保证数据完整。', 'warning')
        _, chart_paths_1 = process_single_image(first_image_stream, live_date_str, live_start_time, configs_to_use[0]); final_chart_paths.update(chart_paths_1)
        for i, file in enumerate(files[1:4], start=1):
            image_stream = io.BytesIO(file.read()); ocr_data, chart_paths = process_single_image(image_stream, live_date_str, live_start_time, configs_to_use[i]);
        ...
        entry.chart_paths = json.dumps(final_chart_paths); db.session.add(entry); db.session.commit()
        flash(f'日期 {live_date_str} (开播于 {live_start_time}) 的数据已成功处理!', 'success'); return redirect(url_for('daily_review', date=live_date_str, start_time=live_start_time))
